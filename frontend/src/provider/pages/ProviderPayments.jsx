    ...
    <ProviderLayout title="Payments" subtitle="Track payouts, status, and settlement history">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-semibold text-slate-900">Stripe</p>
              ...
              <tbody>
                {transactions.map((tx) => (
                  <tr key={tx.id} className="border-t border-slate-100">
                    ...
                    <td className={`px-5 py-3 font-semibold ${tx.tx_type === "CREDIT" ? "text-emerald-700" : "text-red-600"}`}>
                      {tx.tx_type === "CREDIT" ? "+" : "-"}{formatINR(tx.amount)}
                    </td>
                    <td className="px-5 py-3 text-slate-600">{tx.date}</td>
                    <td className="px-5 py-3">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${tx.tx_type === "CREDIT" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>
                        {tx.tx_type}
                      </span>
                    </td>
                  </tr>
