...
export function ProviderReviews() {
  ...
  useEffect(() => {
    if (!user?.id) {
      setLoading(false);
      setError("Provider profile is not loaded.");
      return;
    }
              ...
              {reviews.map((review) => (
                <div key={review.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-slate-900">{review.user_name || review.user || "Customer"}</p>
                      <p className="text-sm text-slate-500">Service: {review.service_name || `#${review.service}`}</p>
                    </div>
