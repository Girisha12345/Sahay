        ...
        <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm font-medium text-slate-700">Price
              <input type="number" min="1" className="mt-1 h-11 w-full rounded-xl border border-slate-300 px-3" placeholder="₹999" required value={form.base_price} onChange={(e) => setForm({ ...form, base_price: e.target.value })} />
            </label>
            <label className="text-sm font-medium text-slate-700">Service duration
              <input type="number" min="15" step="15" className="mt-1 h-11 w-full rounded-xl border border-slate-300 px-3" placeholder="60 mins" required value={form.duration_minutes} onChange={(e) => setForm({ ...form, duration_minutes: e.target.value })} />
            </label>
          ...
          <button type="submit" disabled={saving} className="mt-6 rounded-xl bg-sky-600 px-5 py-3 text-sm font-semibold text-white hover:bg-sky-700 disabled:opacity-50">{saving ? "Saving..." : "Save Service"}</button>
        </form>
      )}
    </ProviderLayout>
  );
