...
function ChatThread({ booking }) {
  ...
  const conversation = messages.map((m) => ({
    ...
    me: String(m.sender_id) === String(user?.id),
  }));
...
export function ProviderMessages() {
  ...
  useEffect(() => {
    bookingService
      .providerBookings()
      .then((res) => {
        const active = (res.data ?? []).filter((b) => ["ACCEPTED", "IN_PROGRESS", "COMPLETED"].includes(b.status));
        setBookings(active);
  ...
  const selectedCustomer = useMemo(() => {
    if (!selected) return "Customer";
    return selected.customer_public
      ? `${selected.customer_public.first_name} ${selected.customer_public.last_name}`.trim() || "Customer"
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
            ...
            <div className="space-y-1">
              {bookings.map((b) => (
                ...
                >
                  <p className="text-sm font-semibold text-slate-900">{b.customer_public ? `${b.customer_public.first_name} ${b.customer_public.last_name}`.trim() : b.customer_name || "Customer"}</p>
                  <p className="text-xs text-slate-500">{b.service_title || b.service_name || "Service"}</p>
                </button>
              ))}
            </div>
          </div>
            ...
            {selected && (
              <>
                <div className="mb-3 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-600">
                  Chatting with <span className="font-semibold text-slate-900">{selectedCustomer}</span>
                </div>
